...
(function (window) {
    ...
    function parseConnectionString(cs) {
        ...
        if (cs[0] !== '/') {
            // Need to add IPv6 support (inside square brackets)
            m = cs.match(/(([^:/?]*)(?::([0-9]+))?)/);
            //m = cs.match(/(([^\[\]:\/?]*)|(\[([0-9a-z]{0,45})\])):([0-9]+)?/);

            if (m) {
                if (m[1]) {
                    result.host = decodeURI(m[1]);
                }
                if (m[2]) {
                    result.hostname = decodeURI(m[2]);
                }
                if (m[3]) {
        ...
        m = cs.match(/\/([^/?]+)/g);
        if (m) {
            result.segments = m.map(decodeURI);
        }
